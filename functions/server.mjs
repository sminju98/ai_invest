...
async function analyzeSecFilingCard({ symbolRaw, filing, companyName }) {
  if (!OPENAI_API_KEY) return null;
  
  const system = [
    "너는 SEC 공시 문서를 분석하는 에이전트다.",
    "공시 정보를 바탕으로 독립적인 분석 카드를 생성한다.",
    "일반 투자자가 이해하기 쉬운 형태로 분석 결과를 제공한다.",
    "",
    "출력은 반드시 유효한 JSON만 출력 (설명, 마크다운, 코드블록 없이):",
    "{",
    "  \"sentiment\": \"positive\" | \"neutral\" | \"caution\",",
    "  \"oneLineSummary\": \"일반 투자자 관점의 한 줄 요약 (숫자/회계 용어 최소화)\",",
    "  \"meaning\": \"투자자에게 의미 (왜 중요한지, 단기/중기 관점에서 어떤 영향을 줄 수 있는지, 2~3줄)\",",
    "  \"risk\": \"주의할 점 (과도한 해석 경계, 리스크 또는 불확실성 요인)\",",
    "  \"finalJudgement\": \"AI 종합 판단 (이 공시가 현재 투자 판단에 긍정/중립/부정 중 어디에 가까운지, 단정적 표현 금지)\"",
    "}",
    "",
    "규칙:",
    "- sentiment는 공시의 전반적인 톤을 반영 (긍정적/중립적/주의 필요)",
    "- oneLineSummary는 일반 투자자가 바로 이해할 수 있는 자연어 문장",
    "- meaning은 구체적이고 실용적인 설명",
  ...
  const user = [
    ...
    `공시 유형: ${filing.form}`,
    `제출일: ${filing.filingDate || "N/A"}`,
    `보고일: ${filing.reportDate || "N/A"}`,
    `설명: ${filing.description || "N/A"}`,
    "",
    "위 공시 정보를 바탕으로 분석 카드를 JSON 형태로 생성하세요."
  ].join("\n");
...
async function handleJudgeStream(req, res) {
  ...
  try {
    ...
    // 4-4) SEC 공시 분석 카드 생성 (독립적으로, 하나의 카드로 통합)
    ...
    // 6) 조건부 종합 판단 + 검증/재시도 (SEC 공시 분석 카드 포함)
    const maxAttempts = 3;
...
// Firebase Functions / other serverless runtimes entrypoint
